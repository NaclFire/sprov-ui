...
install() {
    install_base
    bash <(curl -L -s https://github.com/NaclFire/sprov-ui/raw/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/NaclFire/sprov-ui/raw/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
